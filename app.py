...
# Initialize LanguageTool without the url parameter
...
def submit_quiz():
    ...
    motor_skills = (extracted_object['q7'] + extracted_object['q10'] + extracted_object['q12']) / 12
    auditory = (extracted_object['q1'] + extracted_object['q9']) / 8
    phonics = (extracted_object['q1'] + extracted_object['q5'] + extracted_object['q8'] + extracted_object['q9']) / 16
    processing_speed = (time_value + extracted_object['q11']) / 12

    features = [lang_vocab, memory, motor_skills, auditory, phonics, processing_speed]
    features_array = np.array([features])
    prediction = quiz_model.predict(features_array)
